import React from 'react';
import { FaPlus } from 'react-icons/fa';

const HomePage = () => {
    return (
        <section className='max-w-9/12 mx-auto my-20 border-2  '>
            <div className='text-center my-4 space-y-4  bg-base-100 '>
                <h1 className='text-5xl font-bold'>Friends to keep close in your life</h1>
                <p className='text-gray-500'>Your personal shelf of meaningful connections. Browse, tend, and nurture the
                   <br /> relationships that matter most.</p>
                <div className='flex justify-center items-center pt-4'><button className='btn bg-[#244D3F] text-white flex gap-2 justify-center items-center'><FaPlus />  Add a Friends</button></div>
            </div>
            <div className='flex gap-4 justify-center items-center flex-col md:flex-row pt-10'>
                <div className='h-[150px] w-[250px]  flex flex-col justify-center items-center space-y-2 rounded-xl bg-base-100 shadow-sm'>
                    <p className='text-[#244D3F] font-bold text-2xl'>10</p>
                    <p className='text-gray-500'>Total Friends</p>
                </div>
                <div className='h-[150px] w-[250px]  flex flex-col justify-center items-center space-y-2 rounded-xl  bg-base-100 shadow-sm'>
                    <p className='text-[#244D3F] font-bold text-2xl'>3</p>
                    <p className='text-gray-500'>On Track</p>
                </div>
                <div className='h-[150px] w-[250px]  flex flex-col justify-center items-center  space-y-2 rounded-xl  bg-base-100 shadow-sm'>
                    <p className='text-[#244D3F] font-bold text-2xl'>6</p>
                    <p className='text-gray-500'>Need Attention</p>
                </div>
                <div className='h-[150px] w-[250px]  flex flex-col justify-center items-center space-y-2 rounded-xl  bg-base-100 shadow-sm'>
                    <p className='text-[#244D3F] font-bold text-2xl'>12</p>
                    <p className='text-gray-500'>Interactions This Month</p>
                </div>
            </div>

            {/* <div className='flex gap-4 flex-col justify-center items-center md:flex-row'>
                <div className="card w-150 bg-base-100 card-lg shadow-sm">
                    <div className="card-body text-center">
                        <p className='text-[#244D3F] font-bold text-2xl'>10</p>
                        <p className='text-gray-500'>Total Friends</p>
                    </div>
                </div>
                <div className="card w-150 bg-base-100 card-lg shadow-sm">
                    <div className="card-body text-center">
                        <p className='text-[#244D3F] font-bold text-2xl'>3</p>
                        <p className='text-gray-500'>On Track</p>
                    </div>
                </div>
                <div className="card w-150 bg-base-100 card-lg shadow-sm">
                    <div className="card-body text-center">
                        <p className='text-[#244D3F] font-bold text-2xl'>6</p>
                        <p className='text-gray-500'>Need Attention</p>
                    </div>
                </div>
                <div className="card w-150 bg-base-100 card-lg shadow-sm">
                    <div className="card-body text-center">
                        <p className='text-[#244D3F] font-bold text-2xl'>12</p>
                        <p className='text-gray-500'>Interactions This Month</p>
                    </div>
                </div>
            </div> */}
        </section>
    );
};

export default HomePage;